...
export default function AdminModeration() {
  ...
  const { data: reports = [], isLoading } = useQuery({
    queryKey: ['reports'],
    queryFn: () => base44.entities.Report.filter({ status: 'open' }, '-created_date', 100),
    enabled: user?.role === 'admin',
  });

  const getEntity = (type) =>
    type === 'feedpost' ? base44.entities.FeedPost :
    type === 'comment' ? base44.entities.Comment :
    type === 'mission' ? base44.entities.Mission : null;
  ...
  const removeContent = async (report) => {
    ...
    await base44.entities.Report.update(report.id, { status: 'actioned' });
    queryClient.invalidateQueries({ queryKey: ['reports'] });
