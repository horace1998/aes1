...
const CHANTS = ['FIGHTING! 화이팅!', "You're slaying bestie ✨", 'Proud of you 💜', 'Keep going! 🔥'];
...
export default function CommentsThread({ postId, currentUser, onCountChange }) {
  ...
  const submit = async (content) => {
    ...
    try {
      const post = await base44.entities.FeedPost.get(postId);
      const newCount = (post.comment_count || 0) + 1;
      await base44.entities.FeedPost.update(postId, { comment_count: newCount });
