...
export default function ReportDialog({ isOpen, onClose, targetType, targetId, snapshot }) {
  ...
  const submit = async () => {
    ...
    try {
      const reports = await base44.entities.Report.filter({ target_id: targetId, status: 'open' });
      if (reports.length >= HIDE_THRESHOLD) {
        const entity =
          targetType === 'feedpost' ? base44.entities.FeedPost :
          targetType === 'comment' ? base44.entities.Comment :
          targetType === 'mission' ? base44.entities.Mission : null;
        if (entity) {
