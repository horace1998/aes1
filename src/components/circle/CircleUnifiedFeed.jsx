...
export default function CircleUnifiedFeed({ circleId, members = [], currentUser }) {
      ...
      const memberLists = memberEmails.length
        ? await Promise.all(
            memberEmails.map((email) =>
              base44.entities.FeedPost.filter({ user_email: email }, '-created_date', 20).catch(() => [])
            )
          )
        : [];
      const circleStories = circleId
        ? await base44.entities.FeedPost.filter(
  ...
  const handleLike = async (post) => {
    ...
    await base44.entities.FeedPost.update(post.id, { cheers: nextCheers });
